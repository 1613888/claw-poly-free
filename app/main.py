...
async def api_clawby_status():
    """数据源已切换为免费公共 API(Polymarket + Binance),无需 API Key。"""
    return {"configured": True,
            "masked": "免费公共源",
            "base": "Polymarket + Binance(无需 Key)"}


@app.post("/api/clawby")
async def api_clawby_set(payload: dict):
    """数据源为免费公共 API,无需 API Key;保留端点以兼容前端。"""
    return {"ok": True, "configured": True,
            "message": "已切换为 Polymarket/Binance 免费公共数据源,无需填写 API Key"}

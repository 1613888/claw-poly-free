"""BTC real-time price: Binance spot trade WS + second-level ring buffer
(coverage >= 43 min so a full 6-round horizon plus history fits).

Free public Binance endpoints (no API key):
  WS     wss://stream.binance.com:9443/ws/btcusdt@trade          (last trade px)
  REST   https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT (fallback)
  klines 1s backfill on cold start so a restart is never signal-blind.

When the WS is unreachable (some networks block outbound WSS to Binance), the
REST fallback polls at ~1s so the ring buffer stays dense instead of going
stale — the strategy keeps its 1/s signal cadence on the free REST source.
"""
import asyncio
import json
import logging
...
_buf = deque(maxlen=_BUF_MAX)      # (ts_float, price)
_last_price = 0.0
_last_ts = 0.0
...
async def _rest_refresh():
    """Keep price()/buffer alive via REST (ticker/price) while the WS is down."""
    global _last_price, _last_ts
    try:
        ...
        try:
            async with websockets.connect(url, ping_interval=20, close_timeout=5) as conn:
                ...
                async for raw in conn:
                    d = json.loads(raw)
                    px = float(d.get("p") or 0)          # last trade price
                    if not px:
                        continue
                    now = time.time()
                    _last_price = px
